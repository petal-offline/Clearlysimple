...
export function Header({
  ...
  const renderedMobileActions = mobileActions ?? renderedActions;

  return (
    <>
      <header
        className={cn(
          "fixed inset-x-0 top-0 z-50 mx-auto w-full max-w-5xl border-b border-transparent md:rounded-md md:border md:transition-all md:ease-out",
          {
      ...
      >
        ...
        <div
          className={cn(
            "fixed bottom-0 left-0 right-0 top-14 z-50 flex flex-col overflow-hidden border-y bg-background/90 md:hidden",
            open ? "block" : "hidden"
          )}
        >
          <div
          ...
          >
            ...
            <div className="flex flex-col gap-2" onClick={() => setOpen(false)}>
              {renderedMobileActions}
            </div>
          </div>
        </div>
      </header>
      <div aria-hidden="true" className="h-14 md:h-12" />
    </>
  );
}
